...
| `SubscriptionConfig`                 | struct   | Runtime config for `serve_request_reply`: `max_concurrency` (`0` = unbounded), `max_payload_bytes` (larger messages are dropped with a `warn` log), and an optional `queue_group` that turns the subscription into a load-balanced queue subscribe. Defaults: 16 / 1 MiB / no queue group. |
